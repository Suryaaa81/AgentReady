...
from app.models.catalog import Product, ProductVariant, Inventory
...
def get_products(db: Session, merchant_id: str, limit: int = 100) -> list[Product]:
    return list(db.execute(select(Product).where(Product.merchant_id == merchant_id).limit(limit)).scalars().all())


def search_products_query(db: Session, merchant_id: str, query: str, limit: int = 10) -> list[Product]:
...
def import_catalog_csv(db: Session, merchant_id: str, csv_content: str) -> CatalogImportResult:
    ...
    Expected headers: sku, name, description, category, base_price, currency, variant_sku, variant_attributes, variant_price_override, inventory_available
    """
    reader = csv.DictReader(io.StringIO(csv_content))
    result = CatalogImportResult(products_created=0, products_updated=0, variants_created=0, variants_updated=0, errors=[])
    ...
    for row_idx, row in enumerate(reader):
        try:
            ...
            # Product upsert
            product = db.execute(select(Product).where(Product.merchant_id == merchant_id, Product.sku == sku)).scalar_one_or_none()
            if not product:
                product = Product(
            ...
            if variant_sku:
                # Variant upsert
                variant = db.execute(select(ProductVariant).where(ProductVariant.sku == variant_sku)).scalar_one_or_none()
                ...
                if row.get("variant_attributes"):
                    try:
                        ...
                        result.errors.append(f"Row {row_idx+1}: Invalid JSON in variant_attributes")
                ...
                if not variant:
                    ...
                    # Update inventory
                    inventory = db.execute(select(Inventory).where(Inventory.variant_id == variant.id)).scalar_one_or_none()
