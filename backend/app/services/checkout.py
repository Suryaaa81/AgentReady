...
from app.models.checkout import CheckoutSession, CheckoutItem, PurchaseIntent
from app.models.catalog import ProductVariant, Inventory
from app.schemas.checkout import CheckoutSessionCreate, PurchaseIntentCreate
from app.services.audit import log_event
from app.schemas.audit import AuditEventCreate

def create_checkout(db: Session, merchant_id: str, checkout_in: CheckoutSessionCreate) -> CheckoutSession:
    ...
    for item_in in checkout_in.items:
        ...
        items_to_add.append(CheckoutItem(
    ...
    log_event(db, merchant_id, AuditEventCreate(
        event_type="CHECKOUT_CREATED",
        actor="system",
        payload={"checkout_id": session_obj.id, "total_amount": float(session_obj.total_amount)}
    ), checkout_id=session_obj.id)
    
    return session_obj


def update_checkout_status(db: Session, checkout_id: str, status: str, failure_reason: str = None) -> CheckoutSession:
    ...
    log_event(db, session_obj.merchant_id, AuditEventCreate(
        event_type=f"CHECKOUT_STATUS_{status}",
        actor="system",
        payload={"checkout_id": session_obj.id, "status": status, "failure_reason": failure_reason}
    ), checkout_id=session_obj.id)
    
    return session_obj


def get_checkout(db: Session, checkout_id: str) -> Optional[CheckoutSession]:
    return db.execute(
        select(CheckoutSession).where(CheckoutSession.id == checkout_id)
    ).scalar_one_or_none()
